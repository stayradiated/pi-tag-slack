...
- `[~]` Runtime commands have a shared CLI presentation layer: every successful command emits its direct payload as exactly one JSON value with `--json`, while human mode uses stable concise key/value output, visible empty lists, and visible continuation cursors. JSON failures retain the exact one-value error envelope and human failures remain stderr-only. Runtime `task add` now durably inserts before notifying the daemon-owned pi session, records direct RPC acceptance only after success, and reports notification failures as non-retryable partial success containing the created task ID. Slack list cursors are passed through and common live-Slack failures have basic stable codes. Response-size bounds and comprehensive error mapping remain incomplete.
- `[~]` Setup has a reusable asynchronous validation/staging core with injected validation, prompt, and service seams. Interactive plain setup collects Slack tokens, channel, cwd, pi binary, model/thinking, and an initial trusted user; interactive reset displays affected/backup paths and accepts only typed `RESET`; incomplete-journal recovery is announced and affirmatively confirmed. EOF/cancellation/non-TTY never grants destructive consent. Interactive success installs then starts the user service, while non-interactive success prints those exact next steps. Focused setup interaction tests exist; broader reset failure-injection coverage remains incomplete.
...
- [x] Make `doctor` read-only apart from the unavoidable held lock. Online diagnostics use only daemon control health for database/control/session state. Offline diagnostics require the existing gateway lock without rewriting it and validate a coherent private snapshot of the main database plus any WAL/SHM sidecars. SQLite may recover or checkpoint only the disposable snapshot before `quick_check`, schema-v2, and complete configuration validation; valid WAL-resident commits are included, while uninspectable sidecar combinations fail clearly. Source files are opened read-only/no-follow, opportunistically adding the platform no-atime flag where available; timestamp preservation is best-effort across supported filesystems, while doctor performs no application repair or write to deployment state.
...
- [~] Launch the configured pi binary in persistent RPC mode with the canonical session directory and configured working directory. Daemon startup materializes due/catch-up schedule tasks before pi starts, then sends at most one neutral aggregate recovery summary when durable open inbox/tasks exist; it includes totals and up to three newest rows of each kind and does not alter individual acceptance metadata. Session-reset recovery and process supervision/restart policy are not implemented.
...
- [~] Implement neutral startup/session-reset recovery summaries and the specified no-replay behavior for automatic child restarts. Daemon-start aggregate recovery, including startup-materialized due/catch-up schedule tasks, is implemented; session-reset recovery and child restart behavior remain.
...
- [x] Ordinary assistant RPC output remains session-local: the runtime consumes assistant message frames only as RPC/session events, and Slack posting remains reachable only through explicit control/CLI commands. A focused integration test drives admitted Slack work through the real RPC notification boundary, emits streaming/final assistant output plus `agent_settled`, and verifies no Slack post/upload, inbox resolution, or automatic re-wake occurs.
...
- [~] Implement one-time and Croner-backed recurring schedules, timezone/offset validation, occurrence keys, atomic task creation/advancement, enable/disable/remove behavior, and coalesced downtime catch-up. The first functional system now provides add/list/show/enable/disable/remove, offset-only one-time input, five-field Croner/IANA validation, atomic due-task creation/advance or one-time disable, startup materialization/recovery aggregation, and runtime direct pi notification with acceptance metadata. Recurring downtime now creates one `schedule:<id>:through:<last-UTC>` task with durable first/last/count metadata and advances to the next future run; ordinary and one-time work retain `schedule:<id>:at:<UTC>`. Representative Croner/IANA spring-forward and fall-back coverage plus an atomic advancement failure rollback test exist; broader DST matrices and failure-injection coverage remain.
...
- [~] Implement idle and active confirmed session reset using the stale-safe `<session-id>:<run-sequence>` challenge. `session reset` is coordinator-serialized; an active session returns `CONFIRMATION_REQUIRED` with the exact challenge command and makes no changes. `session reset --confirm` validates/reserves the exact active run, sends its acknowledgement via a control-server post-flush action, and only then terminates/archives it; failed socket flushes cancel the reservation. Add focused socket/fake-child tests for flush ordering, stale challenges, and concurrent new work.
- [~] Archive the old idle session, preserve overrides/open work/tasks/schedules, start a fresh RPC session, and send one recovery summary. Idle reset now uses the shared validated archive-naming primitive, creates a private fresh session directory, handshakes/reapplies desired settings, and sends the aggregate summary without per-item acceptance updates. Dedicated end-to-end reset lifecycle and reset reaction handling coverage remain.
- [x] Implement bounded, cursor-paginated archive list and retention cleanup. Only direct, valid, non-symlink reset archive directories beneath the canonical archive root are eligible; cleanup is coordinator-serialized, uses persisted retention (zero disables it), and identity/lstat-revalidates entries before deletion. Reset bundles and legacy backups outside the archive root are never candidates.

#### Setup, reset, operations, and release

- [~] Rebuild setup as the specified interactive/non-interactive flow. Non-destructive first-time validation covers pi version/RPC catalogs, Slack token/auth/Socket Mode validation, conversation metadata/membership and cosmetic label, trusted-user validation/label, and the explicit trust warning. The interactive collection, cancellation-safe reset confirmation, recovery confirmation, and service handoff are covered; broader end-to-end operational coverage remains.
- [~] Implement `setup --reset`, `--yes`, typed confirmation, collision-free backup bundles, SQLite backup API/WAL handling, manifests/hashes, staging, fsync boundaries, atomic installation, rollback, and sidecar cleanup. Non-interactive `setup --reset --yes` and confirmed interactive reset use the reusable collision-free SQLite-backup bundle and validated fresh config/database/session staging through atomic journaled installation; it fsyncs durability boundaries, validates installed state, cleans WAL/SHM sidecars, and rolls back in-process on ordinary failures.
- [~] Implement reset journaling, startup refusal on an incomplete journal, plain-setup recovery, deterministic restoration, and failure injection after every destructive durability boundary. Versioned, mode-0600 atomic journals are runtime-validated; the installer records all active/staged/rollback and sidecar paths, refuses collisions, preserves bundles, and daemon startup rejects incomplete journals with plain-setup remediation. Plain `setup --yes` and affirmatively confirmed interactive plain setup lock-gate bundle-authoritative recovery from prepared, active-moved, or fresh-installed journals, constrain journal paths, stage atomic restoration, and clean WAL/SHM sidecars. Focused phase/rejection and interactive-confirmation coverage exists; exhaustive boundary injection remains next.
- [~] Implemented bounded local `daemon install|uninstall|start|stop|status|logs` lifecycle commands for systemd user services (`pi-tag-slack.service`) and launchd user agents (`com.stayradiated.pi-tag-slack`), with generated definitions using the public `start` entrypoint, canonical overrides/log paths, injected platform/process/filesystem seams, and focused lifecycle tests. The gateway now has one stderr-only structured pino logger (safe startup default, persisted configured level after SQLite loads), and idempotent ordered bounded shutdown: every asynchronous intake/control/timer/coordinator/pi cleanup component has a deadline; the distinct pi lifecycle deadline exceeds its complete shared stdin-close/TERM/KILL escalation window, accepted coordinator work drains before that escalation, then SQLite and lock cleanup always run. Focused unit coverage exists; log rotation, Slack reconnect policy, metrics, and complete live integration remain out of scope/unimplemented.
- [~] `doctor` now reports every canonical path with type/UID/mode/symlink diagnostics and follows the required online/offline database, session, lock, socket, reset-journal, archive, and media rules. Focused tests cover online no-SQLite behavior, healthy/corrupt/incomplete offline state, unsafe paths, contention, and no mutation. Broader daemon service/status output remains part of the unfinished operations slice.
- [~] README, `.env.example`, and the changelog now document the breaking single-conversation architecture, current CLI, minimal bootstrap environment, local/project trust model, explicit-only Slack communication, manifest reapplication, service reinstall, and state-preserving migration. Complete remaining daemon-definition and agent prompt examples, then re-audit the documentation against the final implementation before release.
...
10. `[~]` Rebuild setup around staged backup bundles, reset journaling, plain-setup recovery, and explicit reset confirmation. Non-interactive destructive installation and journal-based plain `setup --yes` recovery use validated bundles/staging, lock-gated restoration, and startup refusal; interactive reset/recovery confirmation and daemon handoff now exist, with broader failure-injection coverage remaining.
