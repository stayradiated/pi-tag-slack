...
This status was verified against the working tree containing `src/config.ts`, `src/paths.ts`, `src/db.ts`, `src/control.ts`, `src/index.ts`, `src/pi-rpc.ts`, `src/cli/index.ts`, `src/slack.ts`, `src/slack-client.ts`, and `test/gateway-foundation.test.ts`. Formatting, lint, 25 foundation tests, and the TypeScript build passed. Passing that small suite is not evidence that the product contract is complete.
...
- `[~]` Twenty-five foundation tests cover selected schema rejection, database-symlink rejection, sequential event deduplication, no-reopen behavior, limited server framing/ID echoing, task pagination and direct pi delivery/failure metadata, startup recovery summary construction, typed configuration, and configured-conversation Slack history/thread/message/send behavior. Most tests specified later in this document do not exist.
...
- [~] Launch the configured pi binary in persistent RPC mode with the canonical session directory and configured working directory. Daemon startup now sends at most one neutral aggregate recovery summary when durable open inbox/tasks exist; it includes totals and up to three newest rows of each kind and does not alter individual acceptance metadata. Session-reset recovery, scheduler materialization, process supervision/restart policy, and version enforcement are not implemented.
...
- [~] RPC startup sets `one-at-a-time`; Slack deliveries, control-command task creation/direct task notification, and startup recovery dispatch are serialized by one daemon coordinator. Trust changes, scheduler work, reset, and all other mutations are not yet included in that coordinator.
...
- [~] Implement neutral startup/session-reset recovery summaries and the specified no-replay behavior for automatic child restarts. Daemon-start aggregate recovery is implemented; session-reset recovery, scheduler catch-up inclusion, and child restart behavior remain.
...
- [~] Implement the complete task repository and direct notification metadata. Manual task creation/list/show/resolve and direct notification acceptance metadata are implemented; schedules and the remaining task validation/lifecycle coverage are not.
