...
10. `[~]` Rebuild setup around staged backup bundles, reset journaling, plain-setup recovery, and explicit reset confirmation. Non-interactive destructive installation now uses validated bundles/staging, a journal, in-process rollback, and startup refusal; plain-setup recovery and interactive confirmation remain.
