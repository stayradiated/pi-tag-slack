...
- `[~]` First-time non-interactive setup is asynchronous and dependency-injectable. Before staging it validates local typed inputs/working directory, pi version plus a disposable RPC catalog handshake, bot `auth.test`, app Socket Mode credentials, configured conversation access/type/membership and Slack cosmetic label, and initial trusted-user validity/label; it warns about daemon-account influence. It stages a database and bootstrap token file only after those checks. Non-interactive `setup --reset --yes` now publishes validated staging through the journaled in-process rollback core; interactive confirmation and interrupted-journal recovery remain separate and unimplemented.
