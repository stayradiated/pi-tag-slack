...
This status was verified against the working tree containing `src/config.ts`, `src/paths.ts`, `src/db.ts`, `src/scheduler.ts`, `src/control.ts`, `src/index.ts`, `src/pi-rpc.ts`, `src/cli/index.ts`, `src/slack.ts`, `src/slack-client.ts`, and `test/gateway-foundation.test.ts`. Formatting, lint, 28 foundation tests, and the TypeScript build passed. Passing that small suite is not evidence that the product contract is complete.
...
- `[~]` Thirty foundation tests cover selected schema rejection, database-symlink rejection, sequential event deduplication, no-reopen behavior, limited server framing/ID echoing, task pagination and direct pi delivery/failure metadata, startup recovery summary construction, typed configuration, configured-conversation Slack history/thread/message/send behavior, and fake-child RPC session/health snapshots. Most tests specified later in this document do not exist.
...
- [~] Launch the configured pi binary in persistent RPC mode with the canonical session directory and configured working directory. Daemon startup materializes due schedule tasks before pi starts, then sends at most one neutral aggregate recovery summary when durable open inbox/tasks exist; it includes totals and up to three newest rows of each kind and does not alter individual acceptance metadata. Session-reset recovery, coalesced scheduler catch-up, process supervision/restart policy, and version enforcement are not implemented.
- [ ] Enforce the minimum version and perform a runtime-validated capability/model/state handshake.
- [~] The RPC child uses strict byte-level LF JSONL splitting, fatal UTF-8/JSON parsing, command IDs, basic `agent_start`/`agent_settled` state, runtime validation of `get_state`, read-only status snapshots, and stdin-close shutdown. Frame limits, broader response schemas, graceful escalation, restart supervision, and protocol tests remain incomplete.
- [~] RPC startup sets `one-at-a-time`; Slack deliveries, control-command task creation/direct task notification, schedule mutations/runtime ticks, and startup recovery dispatch are serialized by one daemon coordinator. Trust changes, reset, and all other mutations are not yet included in that coordinator.
...
- [~] Implement `session status`, nested model/thinking commands, desired/effective reporting, catalog validation, and immediate-or-safe-boundary application. `session status [--json]` now reports running/stopped, healthy/degraded, session ID, active/idle, run sequence, last process/protocol error, desired configuration, and RPC-exposed effective model/thinking; daemon `health` exposes that same snapshot. Model/thinking mutation, catalog validation, and safe-boundary application remain deferred.
